...
export class Pix {
  ...
  /**
   * O payload Pix gerado automaticamente
   */
  public readonly payload: string;

  constructor(data: PayloadProps) {
    this.data = data;
    this.validate();
  ...
  /**
   * Preenche com zero à esquerda para garantir 2 dígitos
   */
  ...
  private getTransactionAmount(): string {
    if (this.data.amount !== undefined && this.data.amount !== null) {
      const value = Number(this.data.amount).toFixed(2);
  ...
  private getTransactionId(): string {
    if (
      this.data.transactionId !== undefined &&
  ...
  private getCRC16(subject: string): string {
    const calculated = this.calcCRC16CCITT(`${subject}6304`).padStart(4, "0");
  ...
  private validate() {
    ...
    if (!this.data.city) throw new Error("City is mandatory!");
  }
}
